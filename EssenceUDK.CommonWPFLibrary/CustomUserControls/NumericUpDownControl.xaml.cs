...
namespace EssenceUDK.CommonWPFLibrary.CustomUserControls
{
    ...
    public partial class NumericBox : UserControl
    {
        ...
        private void value_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
            ...
            var res = int.TryParse(text, out valParsed);
            if (res && (valParsed > Maximum || valParsed < Minimum))
                e.Handled = true;
            else if (!res)
                e.Handled = true;
            else
        ...
        DependencyObject target, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (numericBox != null) numericBox.TextBoxValue.Text = e.NewValue.ToString();
        }
        ...
        private void OnTextBoxOnTextChanged(object sender, MouseWheelEventArgs e)
        {
            ...
            if (res && (value > Minimum || value < Maximum))
                AssignValue(value);
            if(e.Delta < 0)
                RaiseEvent(new RoutedEventArgs(DecreaseClickedEvent));
            else if(e.Delta > 0)
        ...
        {
            get { return (bool)GetValue(UpButtonEnabledProperty); }
            set { SetValue(UpButtonEnabledProperty, value); }
        }

        // Using a DependencyProperty as the backing store for UpButtonEnabled.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty UpButtonEnabledProperty =
            DependencyProperty.Register("UpButtonEnabled", typeof(bool), typeof(NumericBox), new PropertyMetadata(true));



        public bool DownButtonEnabled
        {
            get { return (bool)GetValue(DownButtonEnabledProperty); }
            set { SetValue(DownButtonEnabledProperty, value); }
        }

        // Using a DependencyProperty as the backing store for DownButtonEnabled.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty DownButtonEnabledProperty =
            DependencyProperty.Register("DownButtonEnabled", typeof(bool), typeof(NumericBox), new PropertyMetadata(true));
