...
namespace EssenceUDK.Platform.DataTypes.FileFormat.Containers
{
	internal class MulContainer : IDataContainer
	{
		...
		public void Dispose()
		{
			if (StreamIdx != null && StreamIdx.CanRead) StreamIdx.Close();
			if (StreamMul != null && StreamMul.CanRead ) StreamMul.Close();
			if (_Chields != null)
				foreach (var member in _Chields.Where(member => member != null))
				{
					member.Dispose();
				}
		}
		...
		{
			return id > IdxTable.Length ? 0 : IdxTable[id].Append;
		}
