...
namespace UltimaXNA.Core.Graphics
{
    public class SpriteBatch3D
    {
        ...
        public SpriteBatch3D(Game game)
        {
            ...
            byte[] bytecode = File.ReadAllBytes("Content/Shaders/IsometricWorld_OPENGL.mgfx");
            //m_effect = new Effect(game.GraphicsDevice, bytecode);
            //var bytes = File.ReadAllBytes("Content/Shaders/VectorRenderer.mgfx");
            m_effect = new Effect(game.GraphicsDevice, bytecode);
